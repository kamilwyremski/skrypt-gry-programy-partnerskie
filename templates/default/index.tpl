...
<div id="top_menu">
	<nav><ul>
		...
		{foreach key=key item=item from=$strony}
			<li><a href="{$base_url}{$item.prosta_nazwa}" title="{$item.nazwa}">{$item.nazwa|truncate:18}</a></li>
		{/foreach}
		...
		{if $ustawienia.wyszukiwarka =='1'}
			<div class="right_box">
				...
				<form action="" method="post">
					<input name="szukaj" type="text" placeholder="Wpisz nazwę" style="width:205px"/>
					<input type="submit" value="Szukaj" style="margin-top: 15px"/>
				</form>
			</div>
		{/if}
