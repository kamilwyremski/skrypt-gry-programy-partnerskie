...
<head>
	...
	<script type="text/javascript" src="{$base_url}templates/{$ustawienia.template}/js/skrollr.min.js"></script>
	{if $slider}
	...
	<div id="top_menu">
		...
		<nav><ul>
			...
			{foreach key=key item=item from=$strony}
				<li><a href="{$base_url}{$item.prosta_nazwa}" title="{$item.nazwa}">{$item.nazwa|truncate:14}</a></li>
			{/foreach}
...
{else}
<div style="margin-top:90px; height: 5px; width: 100%"></div>
{/if}
{if $slider}
...
<div class="site">
	{if $strona == 'main'}
	<div id="wrapper" data-bottom-top="background-position:0 -500px" data-top-bottom="background-position:0 100px" data-smooth-scrolling="off">
		<h1>Gotowy? No to zaczynajmy!</h1>
	</div>
	{/if}
	<div class="left">
		<div class="right_box">
